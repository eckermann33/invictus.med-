...
function generatePDF(d) {
  // Abre uma janela limpa com a ficha formatada e dispara o "Salvar como PDF"
  // do próprio navegador. Confiável (sem tela branca) e sem biblioteca externa.
  ...
  win.document.close();
  toast("Abrindo o PDF — escolha “Salvar como PDF”.");
}
...
function renderCase(c, out, d) {
  const row = (label, val) => (val && String(val).trim())
    ? `<div class="case-row"><span class="case-row__lbl">${label}</span><p>${escapeHTML(val)}</p></div>` : "";
  out.innerHTML = `
    ${c.titulo ? `<h3 class="case-title">${escapeHTML(c.titulo)}</h3>` : ""}
    ...
    ${c.pergunta_raciocinio ? `<div class="case-q"><span class="case-q__lbl">🧠 Para refletir</span><p>${escapeHTML(c.pergunta_raciocinio)}</p></div>` : ""}
